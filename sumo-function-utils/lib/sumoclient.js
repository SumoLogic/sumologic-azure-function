...
SumoClient.prototype.flushBucketToSumo = function(metaKey) {
    ...
    if (targetBuffer) {
        ...
        /*if (headerArray[0]) {
            curOptions.headersObj['X-Sumo-Name'] = headerArray[0];
        }
        if (headerArray[1]) {
            curOptions.headersObj['X-Sumo-Category'] = headerArray[1];
        }
        if (headerArray[2]) {
            curOptions.headersObj['X-Sumo-Host'] = headerArray[2];
        }*/
        let msgArray = [];
        let message;
...
// SECOND STAB USING SIMPLE PROMISE
/*
SumoClient.prototype.flushBucketToSumo = function(metaKey) {
    let targetBuffer = this.dataMap.get(metaKey);
    var self = this;
    ...
    function httpSend(options, messageArray,data,retryCount,success_callback) {
        var sendPromise = new Promise( (resolve,reject) => {
                var req = https.request(curOptions, function (res) {
                    ...
                    res.on('end', function () {
                        if (res.statusCode == 200) {
                            ...
                            reject({'error':null,'res':res,'retry':retryCount});
                        }
                        // TODO: finalizeContext();
                    });
                });
                ...
                req.on('error', function (e) {
                    //self.context.log("Error:"+retryCount);
                    reject({'error':e,'res':null,'retry':retryCount});
                    // TODO: finalizeContext();
                });
                req.write(data);
                req.end();
        ...
        sendPromise.then((body) => {
            self.success_callback(self.context);
        },  (err_obj) => {
                if (err_obj.retry <self.MaxAttempts) {
                    // some failure, could be a 429 or something else, we'll still retry
                    //self.context.log("Retry number: "+err_obj.retry);
                    setTimeout(httpSend,self.RetryInterval,options, messageArray,data, 1+err_obj.retry, self.success_callback);
                } else {
                    //self.context.log("Retried enough, just give up");
                    self.messagesFailed += messageArray.length;
                    self.messagesAttempted += messageArray.length;
                    self.failure_callback(messageArray,self.context);
                }
            }
        );
    }
    ...
    if (targetBuffer) {
        ...
        if (curOptions.compress_data) {
            curOptions.headersObj['Content-Encoding'] = 'gzip';

            zlib.gzip(msgArray.join('\n'),function(e,compressed_data){
                    if (!e)  {
                        httpSend(curOptions, msgArray,compressed_data,0,self.success_callback);
                    } else {
                        self.messagesAttempted += messageArray.length;
                        self.failure_callback(messageArray,self.context);
                    }
            ...
            //self.context.log('Send raw data to Sumo');
            httpSend(curOptions, msgArray,msgArray.join('\n'),0,self.success_callback);
        }
...
*/
// FIRST STAB
/*module.exports.SumoClient.prototype.flushBucketToSumo = function(metaKey) {
 let targetBuffer = this.dataMap.get(metaKey);
 var self = this;
 let curOptions = Object.assign({},this.options);

 function httpSend(options, messageArray,data,retry,success_callback) {
 ...
 if (res.statusCode == 200) {
 self.messagesSent+=messageArray.length;
 self.context.log("Currently sent:"+self.messagesSent+ " messages");
 success_callback(self);
 // TODO: anything here?
 } else {
 if (retry < self.MaxAttempts) {
 // some failure, could be a 429 or not but we'll still do some retry
 self.context.log("Retry number: "+retry);
 setTimeout(sendSumo,self.RetryInterval,options, messageArray, retry + 1, success_callback);
 } else {
 self.context.log("Retried enough, just give up");
 self.messagesFailed += messageArray.length;
 self.failure_callback(messageArray,self);
 }
 }
 // TODO: finalizeContext();
 });
 ...
 function sendSumo(options,messageArray,retryCount,success_callback) {
 if (options.compress_data) {
 options.headersObj['Content-Encoding'] = 'gzip';
 zlib.gzip(messageArray.join('\n'),function(e,compressed_data){
 if (!e)  {
 httpSend(options, messageArray,compressed_data,retryCount,success_callback);
 } else {
 self.failure_callback(messageArray);
 }
 });
 }  else {
 self.context.log('Send raw data to Sumo');
 httpSend(options, messageArray,messageArray.join('\n'),retryCount,success_callback);
 }
 }
 ...
 sendSumo(curOptions,msgArray,0,self.success_callback);
 }
 }*/
